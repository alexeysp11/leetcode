...
Async/await - это механизмы, которые позволяют писать асинхронный код в синхронном стиле. Async используется для обозначения метода, который может выполняться асинхронно, а await используется для ожидания завершения асинхронной операции.
...
Класс `Task` представляет задачу, которую нужно выполнить, а класс `Thread` представляет поток выполнения. `Task` может выполняться на любом доступном потоке из пула потоков, что позволяет более эффективно использовать ресурсы процессора.
...
#### Методы ожидания завершения задачи или потока
...
Объект, который передается в качестве параметра в `lock`, называется *объектом синхронизации* или *монитором*.

В качестве параметра в `lock` передается объект, который будет использоваться для синхронизации доступа к разделяемому ресурсу.

В качестве параметра в `lock` может быть передан не любой объект. Объект должен быть ссылочным типом и поддерживать методы `Equals()` и `GetHashCode()`.

Если к объекту, который передается в качестве параметра в lock, обращаются с двух и более потоков, то один из потоков будет блокирован до тех пор, пока другой поток не освободит монитор. Если один поток будет выполняться очень долго, то это может привести к блокировке других потоков и снижению производительности приложения.
...
### Задачи для понимания разницы между и асинхронными функциями
